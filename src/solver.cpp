...
namespace Tailor
{
    ...
    Solver::Solver(boost::mpi::communicator* comm, const std::vector<std::string>& filename, Profiler* profiler, Partition* partition): verbose_(true), maxtimestep_(10000), comm_(comm), var_exc_(nullptr), donor_var_exc_(nullptr), nsolve_(0), profiler_(profiler), partition_(partition)
    {
        ...
        if (profiler_ != nullptr) {profiler_->start("sol-init");}
        partition_->spc_->init(); // TODO
        //partition_->spc_->init_sod();
        if (profiler_ != nullptr) {profiler_->stop("sol-init");}
    ...
    {
        if (profiler_ != nullptr) {profiler_->start("sol-cae");}
        partition_->spc_->connect_after_exchange(nullptr, "sol");
        if (profiler_ != nullptr) {profiler_->stop("sol-cae");}
    ...
    {
        if (profiler_ != nullptr) {profiler_->start("sol-reset-oga");}
        partition_->spc_->reset_all_oga_status();
        if (profiler_ != nullptr) {profiler_->stop("sol-reset-oga");}
    }
    ...
    bool Solver::repartition()
    {
        ...
        auto aa = [&]()
        {
            ...
            if (profiler_ != nullptr) {profiler_->start("sol-rem-ghostnresi");}
            partition_->spc_->remove_ghosts();
            partition_->spc_->remove_nonresidents();
            if (profiler_ != nullptr) {profiler_->stop("sol-rem-ghostnresi");}
            ...
            if (profiler_ != nullptr) {profiler_->start("sol-reset-status");}
            reset_oga_status();
            if (profiler_ != nullptr) {profiler_->stop("sol-reset-status");}
            ...
            if (profiler_ != nullptr) {profiler_->start("sol-reset-connect");}
            partition_->spc_->reset_mesh_connectivity();
            if (profiler_ != nullptr) {profiler_->stop("sol-reset-connect");}
    ...
    void Solver::exchange()
    {
        ...
        if (profiler_ != nullptr) {profiler_->start("sol-rem-ghost");}
        spc->remove_ghosts();
        if (profiler_ != nullptr) {profiler_->stop("sol-rem-ghost");}

        CellExchanger cell_exc(spc, comm_, true);
        cell_exc.exchange(false, "sol-cell-exc", profiler_, -1);
        //cell_exc.exchange();

        if (profiler_ != nullptr) {profiler_->start("sol-dis");}
        Disconnecter disconnecter(spc, &cell_exc, nullptr);
        disconnecter.disconnect(true, comm_->rank());
        if (profiler_ != nullptr) {profiler_->stop("sol-dis");}

        if (profiler_ != nullptr) {profiler_->start("sol-arr");}
        ArrivalConnecter connecter(spc, &cell_exc.arrival(), nullptr);
        connecter.add_arrival_cells();
        if (profiler_ != nullptr) {profiler_->stop("sol-arr");}
    }
    ...
    void Solver::connect_partition_cells()
    {
        ...
        if (profiler_ != nullptr) {profiler_->start("sol-cpc-dup");}
        nei_exc.remove_dup();
        if (profiler_ != nullptr) {profiler_->stop("sol-cpc-dup");}
    ...
    void Solver::read_settings()
    {
        ...
        desc.add_options()
            ...
            ("solver.tempo_discre", po::value<std::string>()->default_value("back_euler"), "Temporal discretization")
            ...
            ("solver.save-solution", po::value<bool>()->default_value(false), "")
    ...
    void Solver::update_cons_explicitly(Mesh& mesh)
    {
        ...
        else if (torder_ == 2)
        {
            for (MeshCell& mc: mesh.cell_)
            {
                ...
                //mc.cons_sp1_ = mc.cons_s_ + (2./3.) * mc.R_ / ((1/mc.dtao_ + 1/dt_) * mc.poly().volume());
                mc.cons_sp1_ = mc.cons_s_ + mc.R_ / ((1/mc.dtao_) * mc.poly().volume());
                //mc.cons_sp1_ = mc.cons_s_ + mc.R_ / ((1/dt_) * mc.poly().volume());
                mc.prim_ = cons_to_prim(mc.cons_sp1_, fs_.gamma_);
            }
    ...
    void Solver::calc_steady()
    {
        ...
        for (int ntimestep = 0; ntimestep < maxtimestep_; ++ntimestep)
        {
            ...
            if (profiler_ != nullptr) {profiler_->start("sol-1");}
            ...
            if (profiler_ != nullptr) {profiler_->stop("sol-1");}
            
            if (profiler_ != nullptr) {profiler_->bstart("sol-reduce-resi");}
            double maxres;
            boost::mpi::all_reduce(*comm_, local_res, maxres, std::plus<double>());
            if (profiler_ != nullptr) {profiler_->bstop("sol-reduce-resi");}
            
            if (profiler_ != nullptr) {profiler_->start("sol-print-norm");}
            ...
            if (profiler_ != nullptr) {profiler_->stop("sol-print-norm");}
    ...
    void Solver::solve_(int rank)
    {
        ...
        if (profiler_ != nullptr) {profiler_->start("sol-facevel");}
        for (Mesh& mesh: sp.mesh_)
        {
        ...
        if (profiler_ != nullptr) {profiler_->stop("sol-facevel");}
